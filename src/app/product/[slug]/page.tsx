...
export default function ProductPage({params,}: {params: Promise<{ slug: string }>;}) {
    ...
    if (!product) return <div>Product not found</div>;
        ...
        const reduceCount = () => {
            orderCount < 2 ? setOrderCount(1) : setOrderCount(orderCount - 1) 
        }
    return (
        <div className="w-full overflow-x-hidden flex flex-col sm:flex-row items-center sm:justify-center sm:items-start gap-5 lg:gap-10 mt-40 mb-20">
            ...
            <div className="w-4/5 sm:w-[30%] flex flex-col gap-3">
                ...
                <div className="w-full h-1 bg-[var(--progress-bar)] rounded-full">
                    <div className={` rounded-full h-1 ${product.quantity < 6 ? "bg-red-600 h-1/5" : product.quantity < 15 ? "bg-yellow-300 w-3/5" : "bg-green-500 w-full" }`}></div>
                </div>
                ...
                <div className="w-full flex flex-row items-center gap-5">
                    <div className="w-1/3 flex flex-row justify-between items-center border border-[var(--light-gray5)] p-1">
                        <span className="w-full cursor-pointer" onClick={reduceCount}>-</span>
                        <span className="w-full text-center">{orderCount}</span>
                        <span className="w-full text-right cursor-pointer" onClick={() => setOrderCount(orderCount+1)}>+</span>
                    </div>

                    <Button variant="outline" label="Add to cart" />
                </div>

                <div className="w-full lg:w-4/5 mt-5 flex flex-row justify-between md:justify-start items-end gap-4 border-b-3 border-b-[var(--light-gray5))] pb-5 px-2 text-[15px] md:text-[12px]">
                    <p className="flex flex-row items-center gap-1 cursor-pointer"><MdOutlineCompareArrows /> Compare</p>
                    <p className="flex flex-row items-center gap-1 cursor-pointer"><FaShareAlt /> Share</p>
                    <p className="flex flex-row items-center gap-1 cursor-pointer"><AiOutlineQuestionCircle /> Compare</p>
                </div>
            </div>
        </div>
    );
